...
extern unsigned int prim_poly[15];
...
vector<uint32_t> res;
vector<uint32_t> x;
GaloisFieldElement a(&gf, 0);
GaloisFieldElement b(&gf, 0);


vector<uint32_t> solve(Matrix& A) {
    int rows = A.getRows();
    int cols = A.getCols();
    Matrix R0 = Matrix(rows,1,GaloisFieldElement(&gf, 0));
    Matrix R1 = Matrix(rows,1,GaloisFieldElement(&gf, 0));
    Matrix P0 = Matrix(rows,1,GaloisFieldElement(&gf, 0));
    Matrix P1 = Matrix(rows,1,GaloisFieldElement(&gf, 0)); 
    Matrix B(rows,1,GaloisFieldElement(&gf, 0));
    Matrix Delta(rows,rows,GaloisFieldElement(&gf, 0));
    Matrix A2(rows,cols,GaloisFieldElement(&gf, 0));
    Matrix B2(rows,cols,GaloisFieldElement(&gf, 0));
    
    Matrix X0 = Matrix(rows,1,GaloisFieldElement(&gf, 0));
    Matrix X1 = Matrix(rows,1,GaloisFieldElement(&gf, 0));
    
    
    for (int i = 0; i < rows; i++)
        Delta(i, i) = 8192 + rand() % 8192;

    for (int i = 0; i < rows; i++) {
        (X0)(i,0) = rand() % 16384;
    }
    
    A2 = A.transpose() * Delta * Delta * A;
    B2 = A.transpose() * Delta * Delta * B;
    
    R0 = (A2 * X0) + B2;
    P0 = Matrix(R0);
    
    int i = 1;
    int pos_x0 = rand() % rows;
    bool flag = true;

    while (flag) {
        /* calculating */
        a = (R0.transpose() * R0)(0, 0) / (P0.transpose() * A2 * P0)(0, 0);
        X1 = P0*a;      X1 = X1 + X0;
        R1 = A2*P0*a;   R1 = R1 + R0;
        b = (R1.transpose()*R1)(0, 0) / (R0.transpose() * R0)(0, 0);
        P1 = P0*b;      P1 = P1 + R1;
        /* check part */
        Matrix tmp = (A*X1);
...
mpz_class quadraticSieve(const mpz_class& N) {
    ...
    // Smoothness bound B.
    const uint32_t B = MINIMAL_BOUND + ceil(exp(0.6*sqrt(logN * loglogN)));
     ...
     * until we have at least factorBase.size() B-smooth numbers.
     */
    ...
    while (smooth.size() < factorBase.size()) {
        /*
         * Step 2.1.1
         *
        ...
        for (uint32_t i = 0, a = intervalStart; i < INTERVAL_LENGTH; ++i, ++a) {
            if (fabs(logApprox[i]) < threshold) {
                ...
                if (smooth.size() >= factorBase.size())
    ...
    START();
    Matrix M(factorBase.size(), smoothFactors.size(), GaloisFieldElement(&gf, 0));
    for (uint32_t i = 0; i < smoothFactors.size(); ++i) {
        for (uint32_t j = 0; j < smoothFactors[i].size(); ++j) {
            M(smoothFactors[i][j], i) =  M(smoothFactors[i][j], i) + GaloisFieldElement(&gf, 1);
        }
    }
    STOP("init matrix");
    ...
    do {
         ...
         * factor base.
         */
//        cout << "fbase size: " << factorBase.size() << endl;
